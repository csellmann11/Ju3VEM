...
@testset "3D Volume Integration (polyhedron via face triangles)" begin
    ...
    vol_id2 = 1
    ft2 = Triangulation3D.FaceTriangulations3D(topo2)

    # Constant: volume = 1
    m0 = Monomial(1.0, SVector(0,0,0))
    V0 = Triangulation3D.integrate_polynomial_over_volume(m0, vol_id2, topo2, ft2)
    expected_vol = 8.0
    ...
    @test isapprox(Triangulation3D.integrate_polynomial_over_volume(mx, vol_id2, topo2, ft2)/expected_vol, 1.0; rtol=1e-12, atol=1e-12)
    @test isapprox(Triangulation3D.integrate_polynomial_over_volume(my, vol_id2, topo2, ft2)/expected_vol, 1.0; rtol=1e-12, atol=1e-12)
    @test isapprox(Triangulation3D.integrate_polynomial_over_volume(mz, vol_id2, topo2, ft2)/expected_vol, 1.0; rtol=1e-12, atol=1e-12)
    ...
    vol_id = 1
    ft = Triangulation3D.FaceTriangulations3D(topo)

    # Quadratic: ∫ x^2 dV = 1/3, cross term ∫ x*y dV = 1/4
    mxx = Monomial(1.0, SVector(2,0,0))
    mxy = Monomial(1.0, SVector(1,1,0))
    @test isapprox(Triangulation3D.integrate_polynomial_over_volume(mxx, vol_id, topo, ft), 1/3; rtol=1e-12, atol=1e-12)
    @test isapprox(Triangulation3D.integrate_polynomial_over_volume(mxy, vol_id, topo, ft), 1/4; rtol=1e-12, atol=1e-12)
