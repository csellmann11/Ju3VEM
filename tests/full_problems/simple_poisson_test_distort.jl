...
k_global,rhs_global = let 
    ...
    for element in RootIterator{4}(topo)
        ...
        # if abs_volume <= 0
        #     @show element.id
        #     @show cv.volume_data.integrals
        #     @show cv.volume_data.hvol
        #     @show cv.volume_data.vol_bc
        #     display(cv.vnm.map)
        #     # nodes = [cv.mesh[node_id] for (node_id,i) in cv.vnm.map]
        #     nodes = zeros(SVector{3,Float64},length(cv.vnm.map))
        #     for (node_id,i) in cv.vnm.map
        #         nodes[i] = cv.mesh[node_id]
        #     end
        #     @show nodes
        #     throw("abs_volume is zero")
        # end
