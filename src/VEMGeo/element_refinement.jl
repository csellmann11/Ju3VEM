...
function _refine!(edge::Edge{D},topo::Topology{D}) where D
    ...
    if !was_once_refined
        ...
        activate!(get_nodes(topo)[new_node_id],topo)
        for child_id in edge.childs # reactivate childs
            activate!(get_edges(topo)[child_id],topo)
        end
        return new_node_id
    end
...
function _refine!(area::Area{D},topo::Topology{D}) where D
    ...
    if !was_once_refined #standard case
        ...
        activate!(nodes[new_node_id],topo)
        ...
        for child_id in area.childs
            activate!(get_areas(topo)[child_id],topo)
            for edge_id in get_area_edge_ids(topo,child_id)
                activate!(edges[edge_id],topo)
            end
        end
        return new_node_id
    end
    ...
    @no_escape begin
        ...
        new_geo_nodes = @alloc(Int32,2*length(area_nodes))
...
function _refine!(volume::Volume{D},topo::Topology{D};
    ...
    if isempty(volume.childs) # Volume was never refined
        ...
        for child_id in volume.childs
            activate!(get_volumes(topo)[child_id],topo)
            for face_id in get_volume_area_ids(topo, child_id)
                activate!(get_areas(topo)[face_id],topo)
                for edge_id in get_area_edge_ids(topo, face_id)
                    activate!(get_edges(topo)[edge_id],topo)
                end
            end
        end
        return new_node_id
    ...
    fid_to_corner_edge_ids = MutableSmallDict{20,Int32,Tuple{Int32,Int32}}()
    boundary_quad_ids = @MVector zeros(Int32,40)
