...
function _coarsen!(edge::Edge{D}, topo::Topology{D}) where D
    ...
    for child_id in parent.childs
        deactivate!(get_edges(topo)[child_id],topo)
    end
end
...
function _coarsen!(area::Area{D}, topo::Topology{D}) where D
    ...
    for child_id in parent.childs
        deactivate!(get_areas(topo)[child_id],topo)
    end
end
...
function _coarsen!(volume::Volume{D}, topo::Topology{D}) where D
    ...
    deactivate_submanifolds_touching_center!(parent, topo, mid_node_id)
    for child_id in parent.childs
        deactivate!(get_volumes(topo)[child_id],topo)
    end
end
...
function reactivate_parent_and_get_center!(parent::NManifold{N}, topo::Topology{D}) where {N,D}
    make_root!(parent,topo)
    activate!(parent,topo)
    mid_node_id = find_single_intersec(topo.connectivity[1, N], parent.childs)
    deactivate!(get_nodes(topo)[mid_node_id],topo)
    return mid_node_id 
end

function deactivate_submanifolds_touching_center!(parent::Area{D}, topo::Topology{D}, mid_node_id::Integer) where D
    for child_id in parent.childs
