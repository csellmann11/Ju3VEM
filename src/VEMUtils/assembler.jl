...
function get_n_entries(cv::CellValues{3,U},::Int) where U
    n_entries = 0
    # for face_data in values(cv.facedata_col)
    #     n_face_nodes = length(face_data.face_node_ids)
        
    #     n_entries += (n_face_nodes*U)^2
    # end

    # # this is an upper bound, some nodes are shared between faces
    # n_entries
...
function local_ids_to_local_dofs(id::Integer,::DofHandler{D,U}) where {D,U}
